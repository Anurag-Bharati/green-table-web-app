...
import ContentCard from "./MyOrderCard";
...
const MyOrders = () => {
  ...
  const collectionRef = useMemo(() => collection(firestore, "orders"), []);

  useEffect(() => {
    if (status !== "authenticated" || !uid) return;
    const q = query(collectionRef, where("user.id", "==", uid), orderBy("createdAt", "desc"));
    getDocs(q).then((querySnapshot) => {
      if (querySnapshot.empty) return;
      setMyOrders(querySnapshot.docs.map((doc) => ({ id: doc.id, data: doc.data() })));
    });
  ...
  const currentItems = myOrders.slice(itemOffset, endOffset);
  const pageCount = Math.ceil(myOrders.length / itemsPerPage);
  ...
  return (
    ...
    >
      <header className=" relative mx-auto max-w-3xl w-full  md:pl-2 md:pb-4 flex-col sm:flex-row flex justify-between items-center md:h-9">
        <h1 className="text-2xl align-middle mb-3 sm:mb-0">My Orders</h1>
